...
if (!empty($name) && !empty($username) && !empty($email) && !empty($pass)) {
			$pass = md5($pass);
			$query = "INSERT INTO padmin (name, username, email, password, message) VALUES ('$name', '$username', '$email', '$pass', '$message')";
			$results = mysqli_query($db, $query);
			echo "<script>alert('Registered Succesfully, After Verification and Evaluation you will receive an email with your login credentials.')</script>";
		}
		else {
			$errr = "Some fields were not as expected, please verify and try again<br><br><br>";
		}
    } 
?>
...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top" style="padding: 0 0 0 0;">
  ...
  <ul class="navbar-nav justify-content-end">
	...
	<div class="containbox">
		...
		<ul class="navbar-nav">
    <li class="nav-item">
      <a class="nav-link active" href="#" style="font-size: 30px">Administrator Sign Up</a>
    </li>
        </ul>
    </nav>
		<div class="logbox">
		...
		<input type="submit" name="sub" class="submitbtn">
		</form>
	    </div>
	</div>
</body>
